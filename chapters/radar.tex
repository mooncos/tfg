...
With the configuration described in \cref{sec:radar_operating_conditions}, the signals output by the baseband stage from the current radar node are characterised. From the characterisation, the most important aspect is the voltage range of the signal. As the sampling of this signal is later carried out by the acquisition stage, its input range must be compatible with the output voltage range of the IF signal from the baseband stage.

The radar is configured according to \cref{sec:radar_operating_conditions} and put in operation. The I and Q signals are then measured with an Agilent MS ZZZ oscilloscope. Along the descriptions of the baseband signals throughout this document, these IF signals output from the baseband stage are denominated $I_\mathrm{rad}$ and $Q_\mathrm{rad}$ respectively. The measured ramp signals for one period are shown in Figure ZZZ.
