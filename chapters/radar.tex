...
In addition to the IF signals, the baseband stage also outputs trigger signals to allow the ADC in the acquisition stage to determine the beginning of each ramp of the signal. The trigger signals are measured in conjunction with the IF signals with an Agilent MS ZZZ oscilloscope and the data is captured. The measured ramps and trigger signals are shown in Figure ZZZ.

It can be seen that a trigger signal has a high value at the end of the rising ramp and and at the end of the dual ramp. It has a low value elsewhere.
