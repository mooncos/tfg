\chapter{New CWLFM radar node design}\label{sec:continuous-wave-lineal-frequency-modulation-cwlfm-radars-for-gait-analysis}
%\chaptermark{CWLFM radar for gait analysis}
...
\subsection{Radar node configuration} \label{sec:radar_operating_conditions}
...
\begin{figure}[htb]
	\centering
	\includegraphics{radar_eq/fsmin_graphical.pdf}
...
For the purpose of this Bachelor's Thesis, the radar node is configured as follows:
...
\begin{figure}[htb]
	\centering
	\subfloat[Voltage ramp at the input of the VCO. PLL status is labelled during the rising ramp.]{\includegraphics[width=0.5\linewidth]{sucio_graficos/vco/vco_pll.png} \label{fig:vco_rising_ramp}}
	\subfloat[Discarded samples corresponding to instances where the PLL is unlocked.]{\includegraphics[width=0.5\linewidth]{sucio_graficos/vco/vco_discard.png} \label{fig:vco_discard}}
...
It is necessary to discard approximately 10\% of the samples from both ends of each sweep cycle capture: when the PLL is unlocked and a during the dual ramp, as they do not provide information about the CWFM signals. The samples that are discarded are represented in \cref{fig:vco_discard}.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\linewidth]{sucio_graficos/vco/vco_trigger.png} %TODO: cambiar figura
...
The radar is configured according to \cref{sec:radar_operating_conditions} and put in operation. The measured ramp signals for one sweep cycle are shown in \cref{fig:if_raw}.

\begin{figure}[htb]
	\centering
	\subfloat[$I_\mathrm{rad}$]{\includegraphics[width=0.5\linewidth]{sucio_graficos/if_raw/if_raw_i.png} \label{fig:if_raw_i}}
	\subfloat[$Q_\mathrm{rad}$]{\includegraphics[width=0.5\linewidth]{sucio_graficos/if_raw/if_raw_q.png} \label{fig:if_raw_q}}
...
Since the channels are sequentially sampled and all information from both signals needs to be preserved, the limitations of the Nyquist Theorem \cite{Shannon1949} must be met. Therefore,
...
\begin{figure}[htb]
	\centering
	\subfloat[Measurement in absolute units of mV.]{\input{fig/adc/adc_nf_mV.pgf} \label{fig:adc_nf_mv}}
	\subfloat[FFT of the measurement, expressed in logarithmic units of  dBm. A Blackman-Harris window of length 512 has been applied. The frequency resolution of the FFT is \SI{0.975}{\kilo\hertz\per bin}.]{\input{fig/adc/adc_nf_freq.pgf} \label{fig:adc_nf_freq}}
...
\subsection{Signal conditioning, additional IF stage design} \label{sec:signal-conditioning-additional-if-stage-design}
...
\begin{figure}[htb]
	\centering
	\subfloat[10 Hz]{\includegraphics[width=0.5\linewidth]{if/plot_10hz.pdf} \label{fig:if_signal_test_10hz}}
	\subfloat[50 kHz]{\includegraphics[width=0.5\linewidth]{if/plot_50khz.pdf} \label{fig:if_signal_test_50khz}} \\
	\subfloat[50 kHz]{\includegraphics[width=0.5\linewidth]{if/plot_500khz.pdf} \label{fig:if_signal_test_500khz}}
	\subfloat[1 MHz]{\includegraphics[width=0.5\linewidth]{if/plot_1mhz.pdf} \label{fig:if_signal_test_1mhz}}
...
\begin{figure}[htb]
	\centering
	\includegraphics{bode/bode_if_80mhz_manual.pdf}
...
\begin{figure}[htb]
	\centering
	\subfloat[Radar equation interpolation for different target distances.]{\includegraphics[width=0.5\linewidth]{fig/radar_eq/radar_eq.eps} \label{fig:radar_eq_results}} %TODO: vector graphics
	\subfloat[Setup for the measurement.]{\includegraphics[width=0.3\linewidth]{radar_eq/radar_eq_setup.jpg} \label{fig:radar_eq_setup}}
