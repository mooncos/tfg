...
\begin{figure}[h]
	...
	\caption{Circuit of the IF stage signal conditioning step containing an OA. \textit{V\_DC} is the DC-shift provided. The resistors of the feedback networks are responsible of setting the gain configuration. A low-value capacitance is placed at the ouput to provide the input of the ADC with a small amount of loading to reduce influence of high-frequency noise. Two configurations of this circuit are used, one for every component of the IF signal from the baseband radar}
	\label{fig:if_cir}
\end{figure}
